...
<required_reading>
@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/domain-probes.md
@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/gate-prompts.md
@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/universal-anti-patterns.md
</required_reading>
...
| When | Read |
|---|---|
| `--power` in {{GSD_ARGS}} | `workflows/discuss-phase/modes/power.md` (then exit standard flow) |
| `--all` in {{GSD_ARGS}} | `workflows/discuss-phase/modes/all.md` overlay |
| `--auto` in {{GSD_ARGS}} | `workflows/discuss-phase/modes/auto.md` + `workflows/discuss-phase/modes/chain.md` (auto-advance) |
| `--chain` in {{GSD_ARGS}} | `workflows/discuss-phase/modes/default.md` + `workflows/discuss-phase/modes/chain.md` |
| `--text` in {{GSD_ARGS}} or `workflow.text_mode: true` | `workflows/discuss-phase/modes/text.md` overlay |
| `--batch` in {{GSD_ARGS}} | `workflows/discuss-phase/modes/batch.md` overlay |
| `--analyze` in {{GSD_ARGS}} | `workflows/discuss-phase/modes/analyze.md` overlay |
...
**Mode dispatch — Read mode files lazily based on flags in {{GSD_ARGS}}:**

```bash
# Detect advisor mode (file-existence guard — no Read until needed)
if [ -f "C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
- If `--power` in {{GSD_ARGS}}: `Read(workflows/discuss-phase/modes/power.md)` and execute it end-to-end. Do NOT continue with the steps below.
...
```bash
SPIKE_FINDINGS=$(ls ./.codex/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
SKETCH_FINDINGS=$(ls ./.codex/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1 || true)
...
Read `@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
