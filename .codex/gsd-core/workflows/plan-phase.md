...
@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/ui-brand.md
@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/revision-loop.md
@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/gate-prompts.md
@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/agent-contracts.md
@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/gates.md
</required_reading>
...
```bash
_GSD_SHIM_NAME="gsd-tools.cjs"; _GSD_RUNTIME_ROOT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"; GSD_TOOLS="${_GSD_RUNTIME_ROOT}$gsd-core/bin/${_GSD_SHIM_NAME}"; if [ -f "$GSD_TOOLS" ]; then gsd_run() { node "$GSD_TOOLS" "$@"; }; elif [ -f "${_GSD_RUNTIME_ROOT}/.claude/gsd-core/bin/${_GSD_SHIM_NAME}" ]; then GSD_TOOLS="${_GSD_RUNTIME_ROOT}/.claude/gsd-core/bin/${_GSD_SHIM_NAME}"; gsd_run() { node "$GSD_TOOLS" "$@"; }; elif command -v gsd-tools >/dev/null 2>&1; then GSD_TOOLS="$(command -v gsd-tools)"; gsd_run() { "$GSD_TOOLS" "$@"; }; elif [ -f "C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/bin/${_GSD_SHIM_NAME}" ]; then GSD_TOOLS="C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/bin/${_GSD_SHIM_NAME}"; gsd_run() { node "$GSD_TOOLS" "$@"; }; else echo "ERROR: gsd-tools.cjs not found at $GSD_TOOLS and gsd-tools is not on PATH. Run: npx -y @opengsd/gsd-core@latest --claude --local" >&2; exit 1; fi
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/tdd.md` so gate enforcement rules are available during planning.
...
Extract from {{GSD_ARGS}}: phase number (integer or decimal like `2.1`), flags (`--research`, `--skip-research`, `--research-phase <N>`, `--gaps`, `--skip-verify`, `--skip-ui`, `--prd <filepath>`, `--ingest <path-or-glob>`, `--ingest-format <auto|nygard|madr|narrative>`, `--reviews`, `--text`, `--bounce`, `--skip-bounce`, `--chunked`, `--mvp`, `--tdd`, `--force` (override closed-phase gate, see §1.5)).
...
if [[ "{{GSD_ARGS}}" =~ (^|[[:space:]])--tdd([[:space:]]|$) ]]; then TDD_MODE=true; fi
```
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/skeleton-template.md`.
...
**Project skills:** Check .codex/skills/ or .agents/skills/ directory (if either exists) — read SKILL.md files, research should account for project skill patterns
</additional_context>
...
**If found:**
1. Read template: `C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/templates/VALIDATION.md`
...
# project's git root — falling back to git toplevel / $HOME/.codex. Exit codes mirror grep (0=UI,1=none).
_GSD_RT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
UI_GATE_JS=$(for _c in "$_GSD_RT/gsd-core/bin/lib/ui-safety-gate.cjs" "$_GSD_RT/bin/lib/ui-safety-gate.cjs" "$_GSD_RT/.claude/bin/lib/ui-safety-gate.cjs" "C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/bin/lib/ui-safety-gate.cjs" "C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/bin/lib/ui-safety-gate.cjs"; do [ -f "$_c" ] && { echo "$_c"; break; }; done)
...
**If UI-SPEC.md missing AND `--skip-ui` flag is present in {{GSD_ARGS}}:** Skip silently to step 6.
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @C:/WKWK/asiatekoracle/projects/asiastore-gsd/.codex/gsd-core/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
