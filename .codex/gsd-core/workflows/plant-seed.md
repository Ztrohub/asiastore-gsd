...
```bash
if echo "{{GSD_ARGS}}" | grep -qE '\-\-enrich[[:space:]]+SEED-[0-9]+'; then
  ENRICH_TARGET=$(echo "{{GSD_ARGS}}" | grep -oE 'SEED-[0-9]+')
...
If `{{GSD_ARGS}}` is non-empty and contains no `--enrich` flag, treat the full value as `$IDEA` (no prompt).

Only prompt for the idea when `{{GSD_ARGS}}` is empty and no enrich target is present. Store the response as `$IDEA`.
</step>
